...
void main() {
  ...
  testWidgets('shows validation error for password mismatch', (tester) async {
    ...
    await tester.enterText(find.widgetWithText(CustomTextField, 'Password'), '123456');
    await tester.enterText(find.widgetWithText(CustomTextField, 'Confirm Password'), 'password');
    
    await tester.ensureVisible(find.widgetWithText(CustomButton, 'Create Account'));
    await tester.tap(find.widgetWithText(CustomButton, 'Create Account'), warnIfMissed: false);
    await tester.pump();
