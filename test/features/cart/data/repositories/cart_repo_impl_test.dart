...
void main() {
  ...
  group('getCart', () {
    test('should return list of cart items from remote datasource', () async {
      // Arrange
      final tCartItems = [
        CartItemModel(productId: 'p1', quantity: 1, productName: 'Product 1', priceAtTime: 100, productImage: 'img', selectedAttributes: {}),
      ];
      when(() => mockRemoteDataSource.fetchCart()).thenAnswer((_) async => tCartItems);
  ...
  group('addToCart', () {
    test('should call addToCart on remote datasource', () async {
      // Arrange
      when(() => mockRemoteDataSource.addToCart(any(), any(), selectedAttributes: any(named: 'selectedAttributes')))
          .thenAnswer((_) async {});
